...
def canvas(qtbot: QtBot) -> PlotCanvas:
    widget = PlotCanvas(TITLE, X_LABEL, Y_LABEL)
...
def make_projectile_result(multiplier: float) -> ProjectileResult:
    return {
        ...
        "x": multiplier * np.array([0.0, 4.0, 8.0], dtype=np.float64),
        "y": multiplier * np.array([0.0, 3.0, 0.0], dtype=np.float64),
        "vx": multiplier * np.array([4.0, 4.0, 4.0], dtype=np.float64),
        "vy": multiplier * np.array([3.0, 0.0, -3.0], dtype=np.float64),
        "v": multiplier * np.array([5.0, 4.0, 5.0], dtype=np.float64),
        "E": multiplier * np.array([10.0, 8.0, 6.0], dtype=np.float64),
    }
...
def assert_comparison_lines(
    canvas: PlotCanvas,
    results: tuple[ProjectileResult, ProjectileResult, ProjectileResult],
    x_key: str,
    y_key: str,
) -> None:
    ...
    for line, result in zip(canvas.axis.lines, results, strict=True):
        np.testing.assert_array_equal(line.get_xdata(), result[x_key])
        np.testing.assert_array_equal(line.get_ydata(), result[y_key])
...
def test_show_empty_plot_resets_axis_and_redraws(canvas: PlotCanvas) -> None:
    canvas.axis.plot([0.0, 1.0], [0.0, 1.0], label="temporary")
...
def test_finish_plot_applies_formatting_adds_legend_and_redraws(
    ...
    canvas.axis.clear()
    canvas.axis.plot([0.0, 1.0], [0.0, 1.0], label="Example")
    ...
    legend = canvas.axis.get_legend()
    assert legend is not None
    assert [text.get_text() for text in legend.get_texts()] == ["Example"]
...
def test_plot_trajectory_comparison_draws_expected_lines_without_vectors(
    ...
    canvas.axis.plot([99.0], [99.0], label="stale")
    canvas.draw_wind_vector = Mock()
    ...
    assert_comparison_lines(
        canvas,
        (no_drag, linear_drag, quadratic_drag),
        x_key="x",
        y_key="y",
    )
    assert_finished_plot(canvas)
    canvas.draw_wind_vector.assert_not_called()
...
def test_plot_trajectory_comparison_draws_wind_vector_when_enabled(
    ...
    linear_drag = make_projectile_result(2.0)
    quadratic_drag = make_projectile_result(3.0)
    parameters = Parameters(wind_speed=4.0, wind_angle_degrees=30.0)
    ...
    assert_finished_plot(canvas)
    canvas.draw_wind_vector.assert_called_once_with(parameters)


@pytest.mark.parametrize(
    ("method_name", "y_key"),
    [
        ("plot_energy_comparison", "E"),
        ("plot_speed_comparison", "v"),
    ],
)
def test_time_series_comparison_draws_expected_lines(
    ...
    plot_method = getattr(canvas, method_name)
    plot_method(
        no_drag=no_drag,
        linear_drag=linear_drag,
        quadratic_drag=quadratic_drag,
...
@pytest.mark.parametrize("wind_speed", [0.0])
def test_draw_wind_vector_returns_when_wind_is_not_positive(
    canvas: PlotCanvas,
    wind_speed: float,
) -> None:
    canvas.draw_wind_vector(Parameters(wind_speed=wind_speed))

    assert len(canvas.axis.texts) == 0


def test_draw_wind_vector_adds_arrow_and_label(canvas: PlotCanvas) -> None:
