...
def parameters() -> Parameters:
    return Parameters(initial_velocity=20.0, initial_angle_degrees=30.0)
...
@pytest.fixture
def message_boxes(monkeypatch: pytest.MonkeyPatch) -> dict[str, Mock]:
    warning = Mock()
    information = Mock()

    monkeypatch.setattr(main_window_module.QMessageBox, "warning", warning)
    monkeypatch.setattr(main_window_module.QMessageBox, "information", information)
...
def test_main_window_initialization(window: MainWindow, parameters: Parameters) -> None:
...
def test_has_simulation_results_returns_false_when_results_are_missing(
    window: MainWindow,
) -> None:
...
def test_has_simulation_results_returns_true_when_all_results_exist(
    ...
    window.no_drag_result = make_projectile_result()
    window.linear_drag_result = make_projectile_result()
    window.quadratic_drag_result = make_projectile_result()
...
def test_run_simulation_saves_results_and_updates_gui(
    ...
    monkeypatch.setattr(main_window_module, "save_user_settings", save_user_settings)
    monkeypatch.setattr(
        main_window_module,
...
def test_run_simulation_shows_warning_when_parameters_are_invalid(
    ...
    get_parameters.side_effect = None
    get_parameters.return_value = parameters
...
def test_export_plots_warns_when_simulation_was_not_run(
    ...
    plot_motion = Mock()
    monkeypatch.setattr(main_window_module, "plot_motion", plot_motion)
...
def test_export_plots_creates_directory_and_exports_three_plots(
    window: MainWindow,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    parameters: Parameters,
    ...
    monkeypatch.setattr(main_window_module, "plot_motion", plot_motion)
    monkeypatch.setattr(main_window_module, "plot_energy", plot_energy)
    monkeypatch.setattr(main_window_module, "plot_speed", plot_speed)
...
def test_export_animation_warns_when_simulation_was_not_run(
    ...
    animate = Mock()
    monkeypatch.setattr(main_window_module, "animate_projectile_motion", animate)
...
def test_export_animation_creates_directory_and_exports_gif(
    window: MainWindow,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    parameters: Parameters,
...
def test_change_theme_saves_theme_and_updates_application_stylesheet(
    ...
    monkeypatch.setattr(main_window_module, "save_theme", save_theme)
    monkeypatch.setattr(main_window_module, "get_stylesheet", get_stylesheet)
...
def test_open_plots_folder_creates_directory_and_opens_it(
    ...
    open_url = Mock()
    monkeypatch.setattr(main_window_module.QDesktopServices, "openUrl", open_url)
...
def test_close_event_saves_current_parameters(
    window: MainWindow,
    monkeypatch: pytest.MonkeyPatch,
    parameters: Parameters,
) -> None:
